...
if nargout>3
    ...
    [VCVrobust,A,~,scores,~]=robustvcv('tarch_likelihood',parameters,nw,epsilon_augmented,fepsilon,fIepsilon,p,o,q,error_type,tarch_type,back_cast,T);
    % VCV=hess^(-1)/(T-m); % We disregard this step since it mostly results
    % in inverting singular matrices.
    VCV = 0;
