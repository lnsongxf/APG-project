function [sigmahat, Ht, parameters, ll]=DCCcovEst(x, method, demean)
...
if nargin>1
    if strcmp(method,'modelfree')== 0 && strcmp(method,'lineareig') ==0 && strcmp(method,'quEST') ==0
        method = 'QuEST';
    end
end
...
% Determine which shrinkage method to use
...
switch method
    ...
    case 'lineareig'
        % use linear shrinkage to estimate unconditional covariance matrix of
        % devolatilized residuals
        R = linearShrinkage(x./sqrt(H));
    ...
    case 'modelfree'
        % use modelfree shrinkage to estimate unconditional covariance matrix of
        % devolatilized residuals
        R = modelfreeShrinkage(x./sqrt(H));
